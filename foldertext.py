...
說明：
    - 預設輸出「gzip+base64 壓縮塊」（純 ASCII、無行尾空白），因此貼到 codeshare
      / pastebin 這類會「正規化文字」（砍行尾空白、截斷大內容）的平台也不會壞，
      體積還更小。想要肉眼可讀的原文，pack 加 --no-compress。
    - unpack 會自動辨識新（壓縮塊）與舊（純文字）兩種格式，不必指定。
    - 內層純文字格式：文字檔以原文保存、二進位檔自動 base64；內容用「長度前綴」
      記錄，並附 sha256 摘要，還原時自動驗證完整性。
"""
...
MAGIC2 = "FOLDERTEXT v2 gzip"    # 外層壓縮格式（gzip+base64，對 codeshare 竄改免疫）
MARK_FILE = ">>> FILE "
MARK_DIR = ">>> DIR  "
...
def pack(root, use_default_ignore=True, compress=True):
    """把 root 資料夾走訪一遍，回傳整串文字。

    compress=True（預設）時回傳「gzip+base64 壓縮塊」：純 ASCII、無行尾空白，
    貼到 codeshare/pastebin 這類會正規化文字的平台也不會被改壞，而且體積更小。
    compress=False 則回傳肉眼可讀的純文字格式（v1）。
    """
    text = _pack_text(root, use_default_ignore)
    if compress:
        base_name = os.path.basename(os.path.abspath(root).rstrip(os.sep)) or "root"
        return _compress(text, base_name)
    return text


def _pack_text(root, use_default_ignore=True):
    """走訪 root，回傳肉眼可讀的 v1 純文字格式。"""
...
def _compress(text, root_name):
    """把 v1 純文字壓成 gzip+base64 塊。輸出全為 base64 字元 + 換行，
    沒有任何行尾空白/tab/中文，因此貼到會正規化文字的平台也不會壞。"""
    ...
    lines = [
        MAGIC2,
        "# root: %s" % root_name,
        "# sha256: %s" % digest,       # 原始純文字的摘要，還原時驗證
        "# raw-bytes: %d" % len(raw),  # 純參考
        "",
    ]
    # 每 76 字元換一行方便貼上；還原時會忽略所有換行/空白，換不換都無所謂
    lines.extend(b64[i:i + 76] for i in range(0, len(b64), 76))
    sys.stderr.write(
        "已壓縮：純文字 %d bytes -> 分享塊 %d bytes（%.0f%%）。\n"
        % (len(raw), len(b64), len(b64) * 100.0 / max(1, len(raw)))
    )
    return "\n".join(lines) + "\n"
...
def _decompress(content, warnings=None):
    ...
    try:
        ...
        raise SystemExit(
            "錯誤：壓縮塊解不開（內容可能在貼上/傳輸時被截斷或竄改）：%s" % e
        )
...
def _looks_packed(content):
    """判斷一段文字是否為本工具產生的打包內容（新舊格式皆可）。"""
    return (MAGIC2 in content) or (MAGIC in content)
...
def main(argv=None):
    ...
    p_pack.add_argument("--no-compress", action="store_true",
                        help="輸出肉眼可讀的純文字（不做 gzip+base64 壓縮）")
